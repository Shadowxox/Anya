...
def welcomepic(pic, user, chat, id, uname):
    ...
    font = ImageFont.truetype(
        "ShrutiMusic/assets/font.ttf",
        size=45
    )

    draw.text(
        (65, 250),
        f"NAME : {unidecode(user)}",
        fill="white",
        font=font,
    )
...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    if (
        temp.MELCOW
    ).get(f"welcome-{member.chat.id}") is not None:
    ...
    try:
        ...
        # SPECIAL OWNER MESSAGE
        if user.id == config.OWNER_ID:

            caption_text = f"""
<blockquote>
Wᴇʟᴄᴏᴍᴇ ʙᴏss 😸 ᴜ ʀᴇᴀᴄʜᴇᴅ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ...
</blockquote>

<blockquote>
Hᴀᴠᴇ ᴀ ʙᴇᴛᴛᴇʀ ᴛɪᴍᴇ ᴡɪᴛʜ ɴᴇᴡ ғʀɪᴇɴᴅs 😜
</blockquote>
...
👤 <b>ᴜsᴇʀɴᴀᴍᴇ:</b>
@{user.username if user.username else "ɴᴏᴛ sᴇᴛ"}

<b><u>
ʜᴏᴘᴇ ʏᴏᴜ ғɪɴᴅ ɢᴏᴏᴅ ᴠɪʙᴇs,
ɴᴇᴡ ғʀɪᴇɴᴅs,
ᴀɴᴅ ʟᴏᴛs ᴏғ ғᴜɴ ʜᴇʀᴇ!
</u> 🌟</b>
"""
